...
function buildInitialValues(preselectedCourseId: string | null): EnrollmentFormValues {
  const match = courses.find((c) => c.id === preselectedCourseId);
  return {
    studentName: '',
    parentName: '',
    ageClass: '',
    whatsappNumber: '',
    emailAddress: '',
    selectedCourse: (match ?? courses[0]).title,
    message: '',
  };
}
...
function EnrollmentPage() {
  ...
  const [values, setValues] = useState(() => buildInitialValues(searchParams.get('course')));
